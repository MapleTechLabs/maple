...
describe("listRuleChecksQuery", () => {
	it("compiles the minimal query with OrgId + RuleId", () => {
		...
		expect(sql).toContain("ThresholdMode AS thresholdMode")
		expect(sql).toContain("BaselineMedian AS baselineMedian")
		expect(sql).toContain("BaselineLower AS baselineLower")
		expect(sql).toContain("BaselineUpper AS baselineUpper")
		expect(sql).toContain("BaselineBucketCount AS baselineBucketCount")
		expect(sql).toContain("AnomalyScore AS anomalyScore")
		expect(sql).toContain("EffectiveThreshold AS effectiveThreshold")
		expect(sql).toContain("InvestigationId AS investigationId")
