...
/**
 * Collapse a v2 client failure into the error tags the CLI already handles.
 * `bin.ts` dispatches on those tags to decide what is an expected outcome and
 * what closes the root span as an error, so remote failures must arrive tagged
 * rather than as bare decode/transport values. The CLI's own not-found and
 * usage errors pass through unchanged.
 */
export const toWarehouseError =
	(pipeName: string) =>
	(error: unknown): RemoteError =>
		error instanceof WarehouseClientError ||
		error instanceof WarehouseQueryError ||
		error instanceof CliNotFoundError ||
		error instanceof CliUsageError
			? error
			: new WarehouseQueryError({ message: messageOf(error), pipeName, cause: error })
