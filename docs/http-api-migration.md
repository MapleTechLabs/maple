...
- **`onboarding`** left `OnboardingService` without a consumer until the onboarding reward checklist (`/v2/onboarding/checklist`, `OnboardingChecklistService`) became one: it writes `org_onboarding_state` again (creation time on first read, `reward_claimed_at` on claim). `first_data_received_at` still has no writer, so setup-audit's no-data gate remains stale for orgs created after the v1 route went away.
...
| `onboarding`            | Removed outright instead of moved; nothing called it.   See the note above.                                                                                                                                                                                                                                                                              |
| `demo`                  | **Done**: `/internal/demo`.                                                                                                                                                                                                                                                                                                                             |
| `chat`                  | **Done**: `/internal/chat`. The `apply` mutation moved; the four raw chat-session streaming routes stayed where they were. They are a separate top-level `HttpRouter`, not a `MapleApi` group, and they resolve tenancy through `resolveHttpMcpTenant` (which accepts API keys) instead of the HttpApi middleware, so the split cost nothing to make. |
...
Moving a group to `/internal` swaps `Authorization` for `SessionAuthorization`, which refuses API-key-shaped bearers. That is the intended narrowing, and telemetry showed no API-key traffic to any of these paths. It still makes the move a breaking change for any API-key caller.
...
4. **Build the internal tier:** done for `queryEngine`, which now serves from `MapleInternalApi` at `/internal/query-engine` behind `SessionAuthorization`. Telemetry showed one API-key call across all 62 endpoints in 30 days, so the group was moved wholesale instead of split. Billing, demo, digest, and AI triage followed on 2026-08-14. Chat apply followed on 2026-08-15. Still to move: error-agent coordination.
