...
| ----------------------------- | ------------------------------------- | --------------------------------------------------------------- |
| `MAPLE_DISCORD_BOT_TOKEN`     | Bot tab → Token                       | `ingress.requiredConfig` and `outbound.requiredConfig`, by name |
...
The five steps above buy an application; these six are what turn a message in a server into an
answer from Maple — and a proposed change into one that actually happened.
