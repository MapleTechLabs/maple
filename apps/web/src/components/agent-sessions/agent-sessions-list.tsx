...
import { formatCost, sessionLinkWindow, sessionRowIdParts } from "@maple/agent-sessions"
...
export function AgentSessionsList({
	...
	const columns = useMemo<ColumnDef<typeof TABLE_FEATURES, AgentSessionRow>[]>(() => {
		...
		return [
			...
			{
				id: "totalTokens",
				header: sortHeader("Tokens", "totalTokens", "Sent to (in) and generated by (out) the session's model calls"),
				size: 130,
...
/** Which side of the model call each bucket sits on: what the session sent in
 *  (fresh, replayed from cache, written to it) against what it generated. */
 ...
 * the model against what the model generated. The sum is the number the
 * detail page's header reaches; the tooltip carves each side back into the
 * detail page's buckets. A session that reported only a total shows just the
 * total. The two can differ: the index sums the reported figures as stamped,
 * the buckets carve the cache back out of an inclusive prompt figure, and the
 * sort and filter read the index.
 */
function TokenBar({ session }: { session: AgentSessionRow }) {
	...
	// row falls back to the total rather than show the slice as the whole.
	const hasBuckets = drawnTotal > 0 && drawnTotal >= session.totalTokens * BUCKET_COVERAGE_MIN
	const total = hasBuckets ? drawnTotal : session.totalTokens
	// Blank where nothing was reported — a "0" would read as "measured, and it
	// was nothing".
	if (total === 0) return null
	if (!hasBuckets) {
		return (
			<Hint className="font-mono text-xs tabular-nums text-muted-foreground" content={plural(total, "token")}>
				{formatCount(total)}
			</Hint>
		)
	}
	const sides = [
		{ label: "in", title: "Input", tone: "text-chart-tok-input", fill: "bg-chart-tok-input" },
		{ label: "out", title: "Output", tone: "text-chart-tok-reasoning", fill: "bg-chart-tok-reasoning" },
	].map((side) => {
		const parts = drawn.filter((bucket) => INPUT_BUCKETS.has(bucket.key) === (side.label === "in"))
		return { ...side, parts, count: parts.reduce((sum, bucket) => sum + buckets[bucket.key], 0) }
	})
	return (
		<Hint
		...
		>
			{sides.map((side) => (
				<span key={side.label} className="flex items-baseline gap-1.5">
					<span className={cn("min-w-12 text-right", side.count > 0 ? side.tone : "text-muted-foreground/50")}>
