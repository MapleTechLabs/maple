...
Run these from `lib/effect-clickhouse`:
...
`lib/effect-clickhouse/**` changes, reusing the existing ClickHouse 26.2 service.
...
Before release, run **Effect ClickHouse release checks**
(`.github/workflows/effect-clickhouse-release.yml`) on the exact release commit.
