...
> **Relevance to Maple:** Maple is mainly a **consumer/backend** for OTel data. Our ClickHouse/Tinybird schemas, dashboard UI, and query engine all key on semconv attribute names, so drift between the spec and what our ingest/query layers expect causes silent breakage (missing facets, wrong span-status coloring, unmatched filters).
>
> - Our trace UI keys on `http.request.method`/`http.route`/`http.response.status_code`.
> - Our Rust ingest gateway (`apps/ingest`) emits new-semconv HTTP attributes and self-instruments with `service.name="ingest"` (`apps/ingest/src/otel.rs`).
> - Warehouse queries fall back from `db.query.text`/`db.system.name` to the legacy `db.statement`/`db.system` for instrumentations that predate the DB semconv stability migration.
> - Per project convention, our span status values use **title case** (`"Ok"`, `"Error"`, `"Unset"`). This matches the OTel API enum names, not the uppercase `STATUS_CODE_*` OTLP wire enum.
...
- "Names SHOULD be lowercase" and use **namespacing**, delimited by a dot. For example, `http.response.status_code` is the `status_code` attribute in the `http.response` namespace, itself nested under `http`.
- Multi-word components within a dot-delimited segment use **snake_case** (`status_code`, not `statusCode` or `status-code`).
- Character rules: names must be a valid Unicode sequence restricted to printable Basic Latin. Tooling further limits them to lowercase Latin letters, digits, underscore, and dot. Names **must start with a letter and end with an alphanumeric character**, and must not contain two or more consecutive delimiters.
...
- Metric names additionally:
    - Namespaces and metric names generally **SHOULD NOT be pluralized** (`system.process.count`, not `system.processes`).
    - Counters and up-down counters **SHOULD NOT** append `_total`.
    - Units follow UCUM (Unified Code for Units of Measure). Prefer un-prefixed units (`By`, not `MiBy`) and `s` for durations. Bracketed annotations use singular form (`{request}`, not `{requests}`).
...
Source: https://opentelemetry.io/docs/specs/otel/trace/api/ (the core span-naming rule lives in the tracing API spec, which semconv domains implement against)

- "The span name SHOULD be the most general string that identifies a (statistically) interesting class of Spans, rather than individual Span instances." Low cardinality is effectively mandatory. `get_user` is a good name; `get_user/314159` (embedding a user ID) is not.
- Generality wins over human readability when the two conflict.
- Each domain gives its own template (e.g. HTTP: `{method} {route}`; DB: `{db.query.summary}` or `{db.operation.name} {target}`; see domain sections below). Templates are always built from low-cardinality attribute values, never from raw high-cardinality identifiers (full URLs, raw SQL literals, IDs).
...
- Apply `error.type` **consistently** between a span and its corresponding metric for the same operation: same value if failed, absent if succeeded.
- Errors that were retried or handled internally (the operation still completes successfully) **SHOULD NOT** be recorded as errors on the span/metric describing that operation.
...
Per-domain stability (details in each section below):

- **HTTP:** Stable.
- **Database:** Stable (as of the v1.30-era stability migration).
- **Messaging:** Development.
- **RPC:** Release Candidate.
- **Exceptions** as span events: Deprecated in favor of exceptions on logs.
- **Code attributes:** Stable.
- **General attributes** (`server.*`, `client.*`, `network.*`, `url.*`): Stable. A handful of derived `url.*` sub-attributes remain Development.
- **Gen-AI:** Development, and moved to a separate spec repo.
...
When a domain graduates from experimental to stable, its attribute names and values often change (e.g. `http.method` → `http.request.method`). That breaks dashboards and alerts built on the old names, so instrumentations expose an env var that lets users migrate on their own schedule instead of silently flipping conventions:
...
- **Database**: the same three-way shape, with values `database` / `database/dup` / default.
- **Gen-AI**: `gen_ai_latest_experimental` opts into the newest experimental gen-ai shape instead of the pre-1.36.0 one. The default keeps emitting whatever version was previously emitted.
- Constraint: this variable is "only intended to be used when migrating from an experimental semantic convention to its initial stable version." It is not a general-purpose versioning knob. Instrumentations must support the `/dup` dual-emit mode for **at least six months** after introducing it, within their current major version, before dropping the old shape in a later major version.
...
- A **Schema URL** identifies a specific version of a telemetry schema and is the address the schema file can be fetched from: `http[s]://server[:port]/path/<version>`. The portion before the version is the **Schema Family** identifier, stable across all versions in that family.
- OTLP carries `schema_url` at two levels: on `Resource*` messages (e.g. `ResourceSpans`) and on `Scope*` messages (e.g. `ScopeSpans`). They cover different data. The resource-level `schema_url` applies to the `resource` field only; each scope-level `schema_url` applies to that scope's spans/metrics/logs.
- Purpose: semantic conventions evolve (attributes get renamed, e.g. `http.method` → `http.request.method`, or `deployment.environment` → `deployment.environment.name`). A schema file declares the transformation rules needed to convert telemetry between schema versions. A consumer that knows the schema version of incoming data (via `schema_url`) can rewrite it mechanically to the version its dashboards/alerts expect, instead of every consumer hand-rolling compatibility shims.
...
- The **attribute registry** (`/docs/specs/semconv/registry/attributes/`) is an auto-generated, browsable catalog of every registered attribute, organized by namespace (`http.*`, `db.*`, `network.*`, `code.*`, …), each with type, stability, and description. It is the fastest way to check an attribute's current name and stability without reading the full domain spec.
- The registry and all domain specs are generated from YAML model definitions in `github.com/open-telemetry/semantic-conventions` (the source of truth). The rendered docs on opentelemetry.io are a build artifact of that repo. Gen-AI conventions have since split out into their own sibling repo, `github.com/open-telemetry/semantic-conventions-genai` (see the Gen-AI section).

---

## HTTP

**Stability: Stable** (client and server spans/metrics).
...
| `server.port`               | Required       | Client          | (none)                                                                 |
...
### Span status mapping (the key compliance rule)
...
- **5xx**: `Error` for **both** client and server spans, plus `error.type`.
- Client requests cancelled intentionally by the caller should **not** be treated as errors.

This asymmetry (4xx = Unset for SERVER, Error for CLIENT) is the most load-bearing rule for error-rate dashboards. Filtering `StatusCode = 'Error'` on server spans correctly excludes client 4xx noise; the same filter on client spans correctly includes it.

### Retries/resends

Each client resend (redirect, auth failure, 5xx, network error; the cause doesn't matter) increments `http.request.resend_count`, which is set to the ordinal number of the resend attempt.
...
> **Maple hook:** our trace UI keys directly on `http.request.method` / `http.route` / `http.response.status_code`. The SERVER-4xx-is-Unset rule above is what our error-rate widgets assume when they filter on span status.
...
`{target}` prefers, in order: `db.collection.name` → `db.stored_procedure.name` → `db.namespace` → `server.address:server.port`.
...
- Raw (non-parameterized) query text should only be collected **if sanitized to exclude sensitive info**, with literals replaced by a placeholder (e.g. `?`).
- **"Parameterized query text SHOULD NOT be sanitized."** Using parameters is a strong signal from the application that sensitive values are passed out of band as parameters, not embedded in the query text.
...
> **Maple hook:** warehouse queries in `packages/query-engine/src/ch/queries/` (e.g. `service-map.ts`, `errors.ts`) coalesce the stable (`db.query.text`, `db.system.name`) and legacy (`db.statement`, `db.system`) spellings, because spans from older instrumentations still carry the legacy names. This mirrors the dual-emit window the spec expects instrumentations to support.

---

## Messaging

**Stability: Development.** Per the spec, existing instrumentations on v1.24.0-era conventions should not change what they emit by default until this stabilizes.
Source: https://opentelemetry.io/docs/specs/semconv/messaging/messaging-spans/

### Span name

`{messaging.operation.name} {destination}`. `{destination}` prefers `messaging.destination.template` (low-cardinality template), then `messaging.destination.name` (for known, non-temporary destinations), then `server.address:server.port` (only when no specific destination is targeted).
...
`{rpc.method}` if known and not `_OTHER`; otherwise `{rpc.system.name}`.
...
- Recording exceptions as **span events** (event name `exception`) is deprecated: "Use Semantic conventions for exceptions in logs instead." `exception.escaped` is also deprecated: "It's no longer recommended to record exceptions that are handled and do not escape the scope of a span."
- Current guidance: exceptions **SHOULD be recorded as attributes on the LogRecord** passed to `Logger` emit operations, not as span events.
...
- Attributes (same names and semantics in both forms):
...
- Instrumentations **should not** record synthetic exceptions that a framework manufactures purely to represent an error status code.
...
Note: in the v1.29.0 → v1.33.0 migration, the older separate `code.namespace` and `code.function` attributes were folded into `code.function.name`, which now carries the fully-qualified name. All `code.*` attributes are disallowed on Profile signals, since profiling data already captures them.
...
> **Maple hook:** these underpin the HTTP domain above. Our trace UI and the ingest gateway's new-semconv HTTP attributes build on `server.address`/`server.port`/`network.protocol.*`/`url.*` alongside `http.*`.

---

## Gen-AI

**Stability: Development.** Gen-AI semantic conventions **moved out of the main spec** into a dedicated repository, `github.com/open-telemetry/semantic-conventions-genai`. The `opentelemetry.io/docs/specs/semconv/gen-ai/` pages now redirect or point there.
...
The spec covers Inference, Embeddings, Retrieval, Memory, and Execute-Tool spans. Migration is gated by `OTEL_SEMCONV_STABILITY_OPT_IN=gen_ai_latest_experimental` (opt into the newest experimental shape) vs. the default (keep emitting whatever pre-1.36.0 shape was already in use). The spec notes this transition plan will gain a stable value once the gen-ai conventions stabilize.

> **Maple hook:** relevant to our AI features. Treat `gen_ai.*` as pre-stable and expect attribute churn (`gen_ai.system` → `gen_ai.provider.name` already happened). Check the genai repo for the version in use before hard-coding assumptions about the attribute set.
