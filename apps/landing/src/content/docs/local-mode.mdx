...
Maple Local is a free, standalone observability stack for local development — point any OTLP exporter at `localhost`, open the dashboard, and explore traces, logs, metrics, errors, sessions, and a service map with no account, no containers, and nothing to deploy. It is the complete product running on your machine, not a client for the hosted service; the same CLI can talk to a hosted workspace, but nothing here requires one. Everything is single-tenant: every row is stored under `org_id = "local"`.
...
> Prefer to do it by hand? Download a release bundle (both `maple` + `node_modules/chdb`) from [GitHub Releases](https://github.com/MapleTechLabs/maple/releases), or read [`scripts/install.sh`](https://github.com/MapleTechLabs/maple/blob/main/scripts/install.sh) first before piping it to a shell.
