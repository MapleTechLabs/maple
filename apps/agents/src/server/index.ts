...
// process.loadEnvFile is first-wins (it won't overwrite an already-set var), so we
// load in decreasing precedence: this app's local .env first (its AGENTS_URL/PORT
// win over any repo-root value for another project), then repo-root .env.local /
// .env where Maple keeps shared secrets like OPENROUTER_API_KEY.
const ROOT = path.resolve(import.meta.dirname, "../../../..")
const HERE = path.resolve(import.meta.dirname, "../..")
for (const file of [`${HERE}/.env`, `${ROOT}/.env.local`, `${ROOT}/.env`]) {
	try {
...
if (!process.env.OPENROUTER_API_KEY) {
	console.warn(
		"[agents] OPENROUTER_API_KEY is not set — agents will wake but LLM calls will fail. " +
...
const runtime = createRuntimeHandler({
	...
	registry,
	// Local dev: the bundled agents-server isn't configured to sign webhooks, so
	// skip JWKS signature verification (otherwise deliveries are rejected).
...
async function spawnAgent(room: Room, type: string): Promise<string> {
	...
	// Spawn the entity. The endpoint is /_electric/entities/{type}/{id} (PUT);
	// hitting /{type}/{id} directly creates a raw durable stream, not an entity.
	// `initialMessage` is delivered as a system-originated inbox message that
	// triggers the entity's firstWake (where it runs ctx.mkdb() + ctx.observe()
	// and subscribes to the shared chatroom).
	...
	if (!putRes.ok) {
		throw new Error(`Spawn failed: ${await putRes.text()}`)
	}
...
const server = http.createServer(async (req, res) => {
	...
	// Wake delivery from the agents-server.
	if (req.url === "/webhook" && req.method === "POST") {
		console.log(`[webhook] ${new Date().toISOString()} wake received`)
	...
	// Room-scoped routes: /api/rooms/:id/(message|agent)
	const roomMatch = req.url?.match(/^\/api\/rooms\/([^/]+)\/(\w+)$/)
	if (roomMatch) {
		const [, roomId, action] = roomMatch

		// Send a user message — write to shared state; agents wake via observe().
		...
		// Spawn an additional agent in the room.
