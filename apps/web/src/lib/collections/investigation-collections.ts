/**
 * The per-investigation collection, and why it is not part of `OrgCollections`.
 *
 * Every other synced collection is org-wide and there is exactly one live set at
 * a time, because the app shows one org at a time. This one is scoped to a
 ...
 * page. That makes its lifecycle navigation-shaped rather than session-shaped:
 * a new one per investigation opened, torn down once nothing reads it.
 *
...
import { createInvestigationCollection, type InvestigationCollection } from "./investigations"
 ...
 * Navigating between runs tears the previous one down as it drains.
 */
...
export const getInvestigationCollections = (
	...
	if (previous) cleanupCollectionWhenIdle(previous.investigation)
	return current
}
