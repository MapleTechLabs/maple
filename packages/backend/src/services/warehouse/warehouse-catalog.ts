...
const TABLE_NOTES: Record<string, ReadonlyArray<string>> = {
	...
	ai_trace_index: [
		...
		"Rows whose `SpanId` starts with `usage:` are not spans: each is a Claude Code model call's cost, written from its `api_request` log event (migration 0033), with `ResponseId` the call's request id — the same value its `claude_code.llm_request` span carries — and no tokens. Count spans with `SpanId NOT LIKE 'usage:%'`; for a session's cost, take the largest `Cost` per non-empty `ResponseId`, so a call is priced once.",
		"Holds only the agent spans (and usage records), and only their identity — for every span of a detected trace, or for anything it does not carry (`StatusCode`, the tool call's arguments and result, `gen_ai.usage.*` per key), collect `TraceId`s here first, then read `trace_detail_spans` with `TraceId IN (…)` AND a `Timestamp` window.",
