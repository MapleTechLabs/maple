...
const program = Effect.gen(function* () {
	const endpoint = yield* Config.string("CLICKHOUSE_BUILDER_TEST_URL").pipe(Config.withDefault(""))
	yield* check(
		endpoint.trim().length > 0,
		"live-configuration",
		"CLICKHOUSE_BUILDER_TEST_URL is required; refusing to skip live tests.",
	)
