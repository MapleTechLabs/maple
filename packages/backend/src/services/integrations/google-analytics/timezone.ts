...
const formatterFor = (timeZone: string): Intl.DateTimeFormat | null => {
	...
	// An unknown IANA zone (one this runtime's tz database has never heard of) throws. It is
	// reported as null so the caller skips the property rather than filing data under the wrong hour.
	...
	if (Result.isFailure(built)) return null
	formatters.set(timeZone, built.success)
