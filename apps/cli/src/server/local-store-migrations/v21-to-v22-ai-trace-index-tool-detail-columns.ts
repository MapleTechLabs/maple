 ...
 * page read off the raw spans — why a failing call failed (`error.type`, the
 * span's status message, and a failed tool call's result), the fingerprint its
 * failures group by, and what a tool says it does (`gen_ai.tool.description`) —
 * and recreates `ai_trace_index_mv` to fill them, so that page's Errors table,
 * its failure modal's session pane and its header render from the index
 * instead of seeking `trace_detail_spans` across the window's partitions. No
 * row moves and no table is rebuilt.
 *
 ...
 *    columns — metadata-only, defaulting every existing row to `''` or 0.
 ...
 *    `IF NOT EXISTS`.
 *
 * NOTHING IS BACKFILLED, as in 0032: rows materialized under v21 keep `''`/0 for
 * all five — a failure older than the edge groups under the page's `unknown`
...
const dispositions: ReadonlyArray<StateDispositionEntry> = [
	...
	{
		// Existing rows are kept with an empty ErrorType, StatusMessage,
		// ToolDescription and FailedToolCallResult and a zero ErrorFingerprint.
		// The rebuilt MV fills them for spans ingested after the edge.
