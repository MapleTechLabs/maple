/**
 * Running one agent to a structured answer, headlessly.
 *
 ...
 *   completion tool whose parameters *are* the schema — supplied by the feature. The engine decodes
...
import { agentPolicyFor, buildSystemPrompt, type AgentDefinition } from "./agent"
import { buildAgentToolkit } from "./tools"
import { accumulateUsage, makeRunUsage, type RunUsage } from "./usage"
import { type LlmClients, type ResolvedModel, agentSessionSpanAttributes } from "../platform/Llm"
import { ToolExecutor } from "./tool-executor"
import { summarizeCause } from "./failures"
...
): Effect.Effect<AgentPassOutput<S["Type"]>, never, LlmClients | ToolExecutor> =>
	Effect.gen(function* () {
		type A = S["Type"]
		...
		const tools = buildAgentToolkit(toolExecutor, input.agent.permission)
