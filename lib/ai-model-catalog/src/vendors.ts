...
/**
 * Brand colors for vendor slugs. Listed only where the brand has a color we can
 * name with confidence; the rest detect with no color, since a neutral declines
 * to name a brand and a near-miss names the wrong one.
 */
export const VENDOR_BRAND_COLORS: Readonly<Record<string, BrandColor>> = {
