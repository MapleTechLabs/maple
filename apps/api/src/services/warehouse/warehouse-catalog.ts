...
const TABLE_NOTES: Record<string, ReadonlyArray<string>> = {
	...
	ai_trace_index: [
		...
		"`ErrorType` (LowCardinality String, `error.type`), `StatusMessage` and `ToolDescription` (`gen_ai.tool.description`, tool spans only) are the span's own failure reason and tool documentation, truncated at insert — '' on rows materialized before migration 0032. Group failures by `ErrorType` here rather than reading `trace_detail_spans` for it.",
		"Holds only the agent spans, and only their identity — for every span of a detected trace, or for a span attribute it does not carry (the call's payloads, `gen_ai.usage.*` per key), collect `TraceId`s here first, then read `trace_detail_spans` with `TraceId IN (…)` AND a `Timestamp` window.",
