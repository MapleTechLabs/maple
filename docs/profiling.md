...
Status (2026-09-25): still unbuilt. `apps/ingest` now pins `opentelemetry-proto` 0.32, which
also ships the `profiles` feature; it is still not enabled.
...
- **Recommendation:** build it, as a scoped MVP that flattens the pprof-shaped payload
...
| **`packages/browser`**                                           | n/a                                                                                                                            | n/a                                                                               | n/a                                   | Out of scope: browsers expose no sampling profiler worth shipping (JS Self-Profiling API is Chromium-only, origin-trial-grade). |
...
(symbol upload / debuginfod) as explicitly out of scope, and say so in the docs.
...
- **Browser SDK loop-prevention:** `packages/browser` ignores `${endpoint}/v1/`, so a
...
- **API:** `V2ProfilesApiGroup` in `packages/domain/src/http/v2/telemetry.ts` (`/v2/profiles/flamegraph`,
  `/top-functions`, `/timeseries`, `/sample-types`) + `HttpV2ProfilesLive` in
