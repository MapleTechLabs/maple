...
The **OTLP endpoint** is always `https://ingest.maple.dev` and **goes inline** in the bootstrap code. It is not a secret; no env-var indirection is needed.

The **ingest key** is **project-scoped and write-only**. It can only ingest events for one project; it can't read anything or change settings. Treat it like a Sentry DSN, a PostHog public key, or a Datadog RUM client token: **inline it directly in the OTel bootstrap source** alongside the endpoint. Use the public key (`maple_pk_…`) from Settings → Ingestion; it is the one meant to ship in source. No `.env` files, no deploy-target wiring, no `process.env.OTEL_EXPORTER_OTLP_HEADERS`. The user deploys their code and events flow.

Two paths, no questions asked:

### Key in the prompt

If the invoking prompt already contains a Maple ingest key, inline it in every bootstrap file you write, then move on to Step 1.

### No key

Inline the literal sentinel `MAPLE_TEST` in the bootstrap source as a placeholder. Maple's ingest gateway accepts it from anyone and returns 200 without storing anything, so the user's app can boot and exercise the OTel bootstrap path while they get a real key.

Tell the user *briefly* at the top of your work: "I'm using `MAPLE_TEST` as a placeholder so the bootstrap can run. Copy your ingest key from https://app.maple.dev/settings?tab=ingestion (Settings → Ingestion) and search-replace the literal `MAPLE_TEST` in the files I write to swap it in."
...
- **Expo/React Native:** preserve existing Expo Go / unsupported-runtime guards. In supported builds, call `initObservability()` before Sentry and before app registration/user code. Inline the endpoint + ingest key in the observability module, with no `EXPO_PUBLIC_*` env vars. The bootstrap reads them straight from constants.
...
2. **Confirm telemetry leaves the process.** With the inline `MAPLE_TEST` (or real key) in the bootstrap, OTLP POSTs from the dev server should return 2xx. That proves the bootstrap reaches the network. As an extra sanity check, you can `curl -X POST https://ingest.maple.dev/v1/traces -H "authorization: Bearer MAPLE_TEST" -H "content-type: application/json" -d '{}'` and confirm 200. The real signal is the running app's own POSTs succeeding by the time the dev server shuts down. If they don't, the bootstrap is wrong (most often the SDK loads too late, or shutdown doesn't flush).
...
> "The bootstrap currently uses `MAPLE_TEST` as a placeholder so the install could complete end-to-end. Copy your ingest key from https://app.maple.dev (Settings → Ingestion), then search-replace `MAPLE_TEST` in the files I wrote with your new key and redeploy."
...
Suggest installing the Maple MCP server so the agent (Claude Code, Codex, Cursor, etc.) can query telemetry directly the next time the user is debugging: search logs, pull traces, and check error rates from the chat without switching to the dashboard.

For **Claude Code** (most common, and where this skill is running), offer to run it for them:
