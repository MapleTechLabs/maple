 ...
 * (~50 KB). Departures from upstream: the WebGL context is built in a
 * `useMountEffect` (the repo's escape hatch for one-time external sync) and
 * props reach the uniforms from the render loop through a ref, so there is
 * no second, dependency-tracked effect; no CSS file (the host positions the
 * container); and `paused` freezes the clock for reduced motion.
 */
...
export interface GrainientProps {
	...
	color3?: string
	lightMode?: boolean
	/** Stop advancing time (the frame still renders once). */
	paused?: boolean
	className?: string
}
...
export default function Grainient({
	...
	// The latest props, read by the render loop each frame. Assigning a few
	// floats per frame is far cheaper than a dependency-tracked effect, and it
	// keeps the component to one mount-scoped effect.
	const propsRef = useRef<Required<Omit<GrainientProps, "className">>>({
		timeSpeed,
		colorBalance,
		warpStrength,
		warpFrequency,
	...
	useMountEffect(() => {
		...
		const u = program.uniforms as Record<string, Uniform<number | Float32Array>>
		const mesh = new Mesh(gl, { geometry, program })

		const setVec = (name: string, values: number[]) => {
			const arr = u[name].value as Float32Array
		...
		const syncProps = () => {
			...
			setVec("uCenterOffset", [p.centerX, p.centerY])
			u.uZoom.value = p.zoom
			setVec("uColor1", hexToRgb(p.color1))
			setVec("uColor2", hexToRgb(p.color2))
			setVec("uColor3", hexToRgb(p.color3))
