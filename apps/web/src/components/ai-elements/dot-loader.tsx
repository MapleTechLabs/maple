 ...
 *
 * Glyph Pulse is left out for a different reason: it cycles whole motifs over 5.2s, so it reads
 * as a mark flashing between symbols rather than as something moving, which is precisely what
 * this chrome must not do while it sits beside streaming text.
 */
export interface DotLoaderVariant {
	...
	grid: 3 | 5
	/**
	 * How long one pass of this variant's CSS animation takes at `speed` 1, which is the
	 * matrix's 1500ms base loop times whatever multiplier its own rule applies.
	 */
	cycleMs: number
}

export const DOT_LOADER_VARIANTS: ReadonlyArray<DotLoaderVariant> = [
	{ name: "Neon Drift", Component: DotmSquare1, grid: 5, cycleMs: 1500 },
	{ name: "Core Spiral", Component: DotmSquare3, grid: 5, cycleMs: 1500 },
	{ name: "Twin Orbit", Component: DotmSquare4, grid: 5, cycleMs: 1500 },
	{ name: "Prism Sweep", Component: DotmSquare5, grid: 5, cycleMs: 1500 },
	{ name: "Flux Columns", Component: DotmSquare6, grid: 5, cycleMs: 1500 },
	{ name: "Echo Ring", Component: DotmSquare11, grid: 5, cycleMs: 1500 },
	{ name: "Origin Wave", Component: DotmSquare12, grid: 5, cycleMs: 1500 },
	{ name: "Square Spiral", Component: Dotm3x3_1, grid: 3, cycleMs: 1170 },
	{ name: "Drift TR", Component: Dotm3x3_2, grid: 3, cycleMs: 1020 },
	{ name: "Core Echo", Component: Dotm3x3_6, grid: 3, cycleMs: 1230 },
	{ name: "Column Flux", Component: Dotm3x3_7, grid: 3, cycleMs: 1020 },
	{ name: "Row Sweep", Component: Dotm3x3_8, grid: 3, cycleMs: 1020 },
	{ name: "Frame Chase", Component: Dotm3x3_10, grid: 3, cycleMs: 1470 },
	{ name: "Drop Ripple", Component: Dotm3x3_12, grid: 3, cycleMs: 1950 },
	{ name: "Right Surge", Component: Dotm3x3_13, grid: 3, cycleMs: 1020 },
	{ name: "Echo Rings", Component: Dotm3x3_15, grid: 3, cycleMs: 1590 },
]
...
/**
 * How long one pass takes, for every variant. Each one's `speed` is derived from this and its
 * own `cycleMs`, so they all run at the same tempo.
 *
 * The shared tempo is the point, not just the number. Left on their own defaults the pool
 * spanned 0.6s to 1.9s a pass — the fast end twitched at 18px, and a random pick that also
 * randomised tempo made the chrome feel unstable from one turn to the next.
 */
 ...
 */
export function DotLoader({ label, color = "currentColor", size = 18, variant, className }: DotLoaderProps) {
	const [rolled] = useState(
		() => DOT_LOADER_VARIANTS[Math.floor(Math.random() * DOT_LOADER_VARIANTS.length)] as DotLoaderVariant,
	)
	const { Component, grid, cycleMs } = variant ?? rolled
