...
import type { MetricSumRow } from "@maple/backend/services/warehouse/metric-rows"
...
>()("@maple/api/services/GoogleAnalyticsService", {
	make: Effect.gen(function* () {
		...
		const patchRow = (id: string, patch: Partial<GoogleAnalyticsStateRow>) =>
			dbExecute((db) =>
				db.update(googleAnalyticsState).set(patch).where(eq(googleAnalyticsState.id, id)),
			)
				...
				db
					...
					.where(
						and(
							...
							or(
								isNull(googleAnalyticsState.leaseUntil),
								lt(googleAnalyticsState.leaseUntil, msToDate(now)),
							),
						),
					)
		...
		const pollWindow = Effect.fn("GoogleAnalyticsService.pollWindow")(function* (context: {
			...
			let response = yield* runReport({
				...
				request: {
					dimensions: dataset.breakdown ? ["dateHour", dataset.breakdown.dimension] : ["dateHour"],
		...
		const discoverProperties = Effect.fn("GoogleAnalyticsService.discoverProperties")(function* (
			...
			const vanished = existing.filter(
				(row) => row.dataset !== DISCOVERY_DATASET && row.enabled && !live.has(row.propertyId),
			)
		...
		const recoverWindow =
			...
			<R>(
				effect: Effect.Effect<
					number,
					IntegrationsUpstreamError | IntegrationsRevokedError | IntegrationsPersistenceError,
					R
				>,
			) =>
				effect.pipe(
		...
		const pollOrg = Effect.fn("GoogleAnalyticsService.pollOrg")(function* (orgId: OrgId) {
			...
			for (const row of pollable) {
				...
				const fullSweepDue =
					...
					now - row.lastSuccessAt.getTime() >= FULL_RECONCILE_INTERVAL_MS
				const from = Math.max(
					frozenThrough,
					head - (fullSweepDue ? RESTATEMENT_WINDOW_MS : HEAD_WINDOW_MS),
				)
						...
						Effect.gen(function* () {
							...
							if (error instanceof IntegrationsUpstreamError) {
								...
								if (error.status === GA_QUOTA_STATUS) {
									...
									yield* releaseLease(orgId, now + QUOTA_BACKOFF_MS, now).pipe(
										Effect.ignore,
									)
								}
								return FAILED_TICK
		...
		const pollAllOrgs = Effect.fn("GoogleAnalyticsService.pollAllOrgs")(function* () {
			...
			const results = yield* Effect.forEach(
				connections,
				(connection) => pollOrgSafely(connection.orgId),
				{
		...
		const getIntegrationStatus = Effect.fn("GoogleAnalyticsService.getIntegrationStatus")(function* (
			orgId: OrgId,
		) {
				...
				.map(([propertyId, group]): GoogleAnalyticsPropertyStatus => {
					...
					const oldest = (pick: (row: GoogleAnalyticsStateRow) => Date | null) =>
						group
							...
							.reduce<number | null>(
								(min, value) =>
									min === null ? value.getTime() : Math.min(min, value.getTime()),
								null,
							)
					return {
						propertyId,
	...
	static readonly layer = Layer.effect(this, this.make).pipe(
		...
		Layer.provide(Layer.mergeAll(GoogleAnalyticsOAuthService.layer, OrgIngestKeysService.layer)),
	)
}
