				...
				accountSummaries: [
					{
						displayName: "Acme",
						propertySummaries: [
							{ property: `properties/${PROPERTY_ID}`, displayName: "example.com" },
						],
					},
				],
			})
		...
		if (url.includes(":runReport")) {
			...
			return json({
				...
				rows: [...(report ?? new Map())].map(([dateHour, sessions]) => ({
					...
					metricValues: metricNames.map((name) => ({
...
const makeLayer = (testDb: TestDb, fetchOptions: FetchOptions) =>
	Layer.effect(GoogleAnalyticsService, GoogleAnalyticsService.make).pipe(
