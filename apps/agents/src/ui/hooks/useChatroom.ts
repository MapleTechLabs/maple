import { createAgentsClient, db, entities } from "@electric-ax/agents-runtime"
...
// The shared-state stream for a room only exists once an agent's firstWake has
// run ctx.mkdb(), so retry while the first agent boots.
...
/**
 * Observe a chatroom's shared state and the agents in it, live. Returns TanStack DB
 * collections you can drive with useLiveQuery. The browser connects directly to the
 * agents-server (agentsUrl), which streams updates over the durable-streams protocol.
 */
export function useChatroom(agentsUrl: string | null, roomId: string | null) {
	...
	const [agentsCollection, setAgentsCollection] = useState<AgentsCollection | null>(null)
	...
	useEffect(() => {
		...
		async function connect() {
			try {
				...
				const entitiesDb = await client.observe(entities({ tags: { room_id: roomId! } }))
				cleanups.push(() => (entitiesDb as any).close?.())
				const members = (entitiesDb as any).collections.members as AgentsCollection
				...
				const messages = (chatroomDb as any).collections.messages as MessagesCollection
				if (!cancelled) {
