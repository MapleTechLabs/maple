...
Review a diff, PR, or named file of *Maple's own code* against the *OpenTelemetry specification* as snapshotted in `docs/otel-spec/` (spec **v1.58.0**, researched 2026-07-05). The deliverable is spec findings: MUST/SHOULD violations with normative citations.

Neighbouring skills cover other ground:

- `maple-audit`: whole-project instrumentation-quality audit against Maple conventions, mostly for external projects.
- `maple-telemetry-conventions`: the house-conventions reference. Cross-reference it here; never enforce it from here.
- `/code-review`: generic diff correctness.

`docs/otel-spec/README.md` is the routing hub. The nine spec files beside it are the **only authority this skill may cite**. Every section in them carries an inline `Source:` deep link to the official spec; findings copy that link.
...
| `warn`     | Violates a SHOULD / SHOULD NOT, or newly relies on a deprecated path: new span-event usage for event-shaped data, deprecated semconv keys without coalescing, missing throttle hints on 429/503.                               |
...
Map what the diff touches to the minimal spec sections. Route by heading text, never by line numbers. The surfaces are the three from `docs/otel-spec/README.md`: **1** ingest gateway as OTLP server · **2** self-instrumentation as SDK consumer · **3** UI/query as data consumer.
...
| Consumers of status / severity / db keys: `WarehouseQueryService` (`packages/backend/src/services/warehouse/`), `packages/query-engine`, `packages/domain/src/tinybird/`, UI classification code | 3 | semantic-conventions.md → matching domain section (HTTP status rule: `Span status mapping (the key compliance rule)`); logs.md → `2. Log Data Model` (esp. `2.2 SeverityNumber reference table`), `5. Events` |
...
| Anything OTel-shaped that doesn't map above | none | README `Files` table → pick ONE file; read its `Relevance to Maple` intro (where present), then decide which sections to load |

**Loading rule:** run `Grep -n '^##' docs/otel-spec/<file>.md` to locate the routed heading, then `Read` with offset/limit from that line to the next same-level heading. If the file has a `Relevance to Maple` intro (traces, metrics, logs, resource-and-config, context-propagation, emerging-and-compat), read that first (~30 lines) for orientation.

**Budget:** never read a spec file end-to-end (traces.md is ~800 lines). Load at most 2 spec files per review by default, section-scoped. If a third is genuinely needed, say why in the report's "Spec sections loaded" line.
...
| 4 | 1 | Every 4xx/5xx response body MUST be a protobuf `Status`. | otlp.md § HTTP status codes: retryable matrix |
| 5 | 1 | Server MUST accept gzip-compressed request bodies. | otlp.md § Transports: gRPC vs HTTP → Compression |
| 6 | 1 | OTLP/JSON: hex trace/span IDs, lowerCamelCase field names, enums accepted as numbers, unknown fields MUST be ignored. | otlp.md § JSON encoding (OTLP/HTTP JSON): deviations from standard protobuf JSON |
...
| 8 | 2 | API/SDK calls MUST NOT throw into the host application. Fail safe and self-diagnose to the SDK's own log. | stability-and-compliance.md § 5. Error-handling principles (fail-safe requirements) |
| 9 | 2 | HTTP SERVER span status = `Error` only for 5xx; 4xx leaves server-span status `Unset`. (CLIENT spans: `Error` on any 4xx/5xx.) | semantic-conventions.md § HTTP → Span status mapping (the key compliance rule) |
...
| 12 | 2 | Resource is immutable once the provider is built; on merge, the updating resource wins per key. | resource-and-config.md § 1. Resource: definition, immutability, merge rules |
...
| 16 | 1/2 | All-zero `trace-id`/`parent-id` in `traceparent` is invalid: treat it as absent and start fresh. Extraction never throws; malformed input degrades to "no context extracted". | context-propagation.md § traceparent → Practical MUSTs for an ingest gateway |

## Step 1: Scope the diff

Resolve what's under review: the working diff (`git diff` / `git diff --staged`), a PR (its diff via the available GitHub tooling), a commit (`git show <sha>`), or files the user named. List the touched files grouped by path. If nothing touches a compliance surface or OTel-shaped content (no spans, exporters, OTLP handling, telemetry attributes, or telemetry consumers), say so in two lines and stop. Don't spec-review generic code.
...
packages/backend/src/services/warehouse/WarehouseQueryService.ts → surface 3 → semantic-conventions.md §Database
```

## Step 3: Load minimally

Check the quick table first. If it covers every implicated rule and no finding looks `critical`, skip spec loading entirely. Otherwise grep-then-read only the routed sections per the loading rule. Track what you loaded; it goes in the report header.

## Step 4: Review

Walk the diff hunks against the loaded normative text. For each candidate finding, quote the normative sentence to yourself, take severity from its MUST/SHOULD/MAY word and the section's stability label, and record evidence as `file:line` in the diff. Keep two piles: spec findings, and house-convention observations (Maple-specific rules from `maple-telemetry-conventions` that the spec doesn't mandate).
...
Fix in severity order. *What* to change comes from the cited spec section. *How* to write it defers to the style skills (`maple-rust-style` for `apps/ingest`, `maple-effect-style` for `apps/api`) and to `maple-telemetry-conventions` for attribute spellings. After edits, re-verify that the touched app still builds (`bun typecheck` for TS, `cargo check` for `apps/ingest`). A fix that breaks the build is a regression: fix or revert it.
...
- The README known-gap items (Jaeger `uber-trace-id`, W3C Baggage document status, global metrics cardinality-limit env var, env-var vs declarative-config precedence, semconv naming/registry landing pages) are capped at `info` and tagged "unverified". Re-check the canonical URL before asserting them as fact.
- Documented intentional Maple deviations are **not** findings: Title Case `"Ok"/"Error"/"Unset"` status storage (one-time translation at the OTLP boundary), the `deployment.environment` dual-emit migration bridge, and read-side coalescing of `db.statement`→`db.query.text` and `db.system`→`db.system.name` (all in README § Maple's three compliance surfaces). Flag one only if the diff *breaks* the documented pattern.
...
- Respect the loading budget: section-scoped reads only, at most 2 spec files by default, never a full file.
