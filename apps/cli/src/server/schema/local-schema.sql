...
-- projectRevision: 2114217b5321374b646a3998e59fc8508be4935715c957582ebe5f652200724a
...
CREATE TABLE IF NOT EXISTS alert_checks (
    ...
    ThresholdMode LowCardinality(String) DEFAULT 'static',
    BaselineMedian Nullable(Float64),
    BaselineLower Nullable(Float64),
    BaselineUpper Nullable(Float64),
    BaselineBucketCount UInt16 DEFAULT 0,
    AnomalyScore Nullable(Float64),
    EffectiveThreshold Nullable(Float64),
...
CREATE TABLE IF NOT EXISTS service_health_hourly (
    OrgId LowCardinality(String),
    Hour DateTime,
    ServiceName LowCardinality(String),
    DeploymentEnv LowCardinality(String),
    ...
    ApdexSatisfiedCount SimpleAggregateFunction(sum, Float64),
    ApdexToleratingCount SimpleAggregateFunction(sum, Float64)
)
ENGINE = AggregatingMergeTree
PARTITION BY toDate(Hour)
...
CREATE MATERIALIZED VIEW IF NOT EXISTS service_health_hourly_mv TO service_health_hourly AS
SELECT
          OrgId,
          toStartOfHour(toDateTime(Timestamp)) AS Hour,
          ServiceName,
...
SELECT
  ...
  nullIf(if(coalesce(nullIf(SpanAttributes['db.query.text'], ''), SpanAttributes['db.statement']) != '', trimBoth(concat(upperUTF8(extract(coalesce(nullIf(SpanAttributes['db.query.text'], ''), SpanAttributes['db.statement']), '^\\s*(\\w+)')), if(extract(coalesce(nullIf(SpanAttributes['db.query.text'], ''), SpanAttributes['db.statement']), '(?i)(?:from|into|update|join|table)\\s+\\W?([\\w.]+)') != '', concat(' ', extract(coalesce(nullIf(SpanAttributes['db.query.text'], ''), SpanAttributes['db.statement']), '(?i)(?:from|into|update|join|table)\\s+\\W?([\\w.]+)')), ''))), ''), ''),
