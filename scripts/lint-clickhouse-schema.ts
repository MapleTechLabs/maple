...
for (const stmt of latestSnapshotStatements) {
	...
	if (match) {
		violations.push({
			...
			message: `Statement uses ${match[1]}(), which is missing from local mode's non-ICU chDB engine. Use ${match[1] === "upperUTF8" ? "upper" : "lower"}() (identical for ASCII) or an ICU-free formulation. Statement: ${stmt.split("\n")[0]?.slice(0, 120)}`,
		})
	}
}
