 ...
 * Untraced: the session id, kind and window source belong on the read's own
 * span, where they are still there when the read itself fails.
 */
const resolveRead = Effect.fnUntraced(function* (
	tenant: TenantContext,
	payload: {
...
const foldSummary = (
	...
	const turns: AiSessionTurnSummary[] = kept.map((row) => {
		...
		return {
			...
			...(turnUsage.cost !== undefined && { cost: turnUsage.cost }),
			models: row.models,
			agentNames: row.agentNames,
		}
	})
