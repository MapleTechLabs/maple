...
import { AuditEventsQueueProducer, QueueSendError } from "@maple/backend/platform/bindings"
	...
	it.effect("writes one audit_log row through ingest, with '' for absent values", () =>
		Effect.gen(function* () {
			...
			yield* Effect.gen(function* () {
				...
				Effect.provide(
					Layer.effect(AuditLogService, AuditLogService.make).pipe(Layer.provide(warehouse.layer)),
				),
			)
	...
	it.effect("publishes to the audit queue instead of writing when the binding is present", () =>
		Effect.gen(function* () {
			...
			yield* Effect.gen(function* () {
				...
				Effect.provide(
					Layer.effect(AuditLogService, AuditLogService.make).pipe(
