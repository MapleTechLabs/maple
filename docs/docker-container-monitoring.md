...
page, and an Infrastructure tab on spans/logs, plus the design decisions that will bite if
...
2. **Ship & store.** OTLP HTTP → ingest gateway → warehouse, untouched: no gateway changes, no
   ...
   `metrics_sum`, identity in the raw `ResourceAttributes` map. Identity is read at query time,
   exactly like the pod queries.
...
4. **Serve.** Five container endpoints under `/internal/query-engine`: `list-containers`,
   `containers-summary`, `container-detail-summary`, `container-infra-timeseries`, and
   `container-facets`. Handlers are in `apps/api/src/routes/internal/query-engine.http.ts`,
   contracts in `packages/domain/src/http/query-engine.ts`.
5. **Render.** `/infra/containers` list + `/infra/containers/$containerName?host=` detail,
   ...
   (`packages/backend/src/dashboard-templates/infrastructure/docker-containers.ts`).
...
- **Percent scale is normalized in the queries.** docker_stats percents are 0..100; every
  ...
  the pod pages. CPU can legitimately exceed 1.0 on multi-core containers. This is documented and
  left unclamped. The dashboard template charts the raw 0..100 gauge and says so in the widget title.
...
- **Sum-family aggregation is per-metric.** Cumulative counters (network, block I/O) surface
  bucketed sums (relative shape, same caveat as the host network chart). Sampled sums such as
...
- **Scopes are `saturated | elevated | stale`, deliberately with no `unbounded`.** Running without
...
- `@maple/effect-sdk` auto-detects `container.id`/`container.runtime` (mountinfo → cgroup →
  short-id hostname, via `process.getBuiltinModule` so edge bundles stay safe). It does not
  detect `container.name`, so auto-detection alone does not light the Infrastructure tab.
...
`otel-collector-maple:0.2.0`, published from the `otel-collector-maple-v0.2.0` tag by
`.github/workflows/publish-otel-collector-maple.yml`. The workflow is tag-driven: merging to main
publishes nothing.
...
Before announcing, confirm the package is public with an anonymous pull. Then smoke-test one live
agent: `container.cpu.utilization` rows should arrive through the hosted gateway, and the compose
project/service facets should populate. The query tests assert SQL strings only, so they cannot
prove that path. The BYO-ClickHouse `renderCollectorYaml()` tracks the latest _published_ tag.

## Not built yet, and why

`service_platforms_hourly` ContainerId/ContainerName columns + the `"docker"` service-map
platform badge (a warehouse migration; read [warehouse-rollups.md](warehouse-rollups.md) first),
