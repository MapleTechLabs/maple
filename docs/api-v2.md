...
The Maple v2 API is the public, documented, stability-committed HTTP surface for customer-stable Maple resources and workflows. It follows Stripe's API design: resource-oriented URLs, prefixed object IDs, uniform list/error envelopes, and scoped keys. It departs from Stripe's v1 mechanics where those are legacy (JSON PATCH updates instead of form-encoded POST, ISO-8601 timestamps instead of epoch seconds).

The **executable contract is the spec**: `MapleApiV2` in `packages/domain/src/http/v2/` (an Effect `HttpApi`). OpenAPI is derived from it automatically and served as an interactive reference at **`/v2/docs`**. This document holds the design guidelines every v2 contract file must follow, plus the roadmap for the full surface.
...
Within that group the role requirement is not uniform. Install, settings, unlink and the destinations listing require the org-admin role; the connector list does not, because the dashboard renders link state for every member. The destinations listing is gated because it lists channels the bot can reach, which is workspace membership information rather than Maple state.
...
- `param` names the offending parameter when applicable. On a request-decode failure it carries the full JSON path of the bad value (`widgets[3].display.chart_presentation.fill_nulls`), which is what identifies the offending widget. The `message` also names the enclosing widget's `id` when the failure reports the value it came from; a struct or array decode failure does not, so most of these read as the path alone.
...
The limiter (`packages/backend/src/services/auth/ApiV2RateLimiter.ts`) fails open: if its binding is missing or errors, Maple logs and traces `maple.rate_limit.outcome=failed_open` and continues the request. Cloudflare counters are local to the location serving the Worker and eventually consistent, so this is abuse protection rather than exact global quota accounting.
...
| `error_issues` 🟡                    | list/retrieve/`service_counts` ✅; `events`, `comments`, `transitions`, `assignee`, `severity` deferred                                                 | `errors`                                                                        |
...
| `onboarding/checklist` ✅            | retrieve + claim (singleton; steps verified per request, claim admin-only and idempotent)                                              | `OnboardingChecklistService`                                                    |
| `scrape_targets` ✅                  | CRUD + `probe` + `checks`                                                                                                              | `ScrapeTargetsService`                                                          |
| `attribute_mappings` ✅              | CRUD                                                                                                                                   | `IngestAttributeMappingService`                                                 |
| `integrations/chat_*` ✅             | connector list (manifest + linked workspaces) + admin-only install/settings/unlink; the chat platform is a value, never a path segment | `ChatWorkspaceService`                                                          |
...
| `services` ✅                        | `GET /v2/services`, `GET /v2/services/{name}`, `GET /v2/services/{name}/overview` (summary + all-signal series + operations, one round-trip) | `queryEngine`                                                                   |
| `service_map` ✅                     | `GET /v2/service_map`                                                                                                                  | `queryEngine`                                                                   |

Also mounted, outside this table: `audit_log` (list), `environments`, `instrumentation/signals` (retrieve), `widget_credentials` (mint/revoke), `widget_summary` (retrieve), and `share` (share-link reads). The contracts are in `packages/domain/src/http/v2/`.
...
2. Add the group to `MapleApiV2` in `packages/domain/src/http/v2/api.ts` and export it from `v2/index.ts`.
