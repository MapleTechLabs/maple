...
const commitLayer = (testDb: TestDb, http: Layer.Layer<GithubHttp>) => {
	...
	const provider = Layer.effect(GithubProvider, GithubProvider.make).pipe(
		Layer.provide(Layer.mergeAll(env, githubAppClient)),
	)
	const registry = Layer.effect(VcsProviderRegistry, VcsProviderRegistry.make).pipe(Layer.provide(provider))
	const service = Layer.effect(VcsCommitService, VcsCommitService.make).pipe(
		Layer.provide(Layer.mergeAll(data, registry)),
	)
