...
describe("AlertReadModelsService boundary", () => {
	it("depends on persistence and warehouse reads, not scheduler or delivery capabilities", () => {
		const imports = importSpecifiers(
			readModule("../../../../packages/backend/src/services/alerts/AlertReadModelsService.ts"),
		)

		expect(imports).toContain("@maple/backend/platform/DatabaseLive")
		expect(imports).toContain("@maple/backend/services/warehouse/WarehouseQueryService")
	...
	it("is the capability consumed by the incident and delivery read handlers", () => {
		...
		expect(alertRulesImports).toContain("@maple/backend/services/alerts/AlertReadModelsService")
		expect(alertRulesImports).toContain("@maple/backend/services/alerts/AlertsService")
	})
})
