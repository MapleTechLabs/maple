 ...
 * jar, the setter records the write instead of applying it (the browser would silently
...
afterEach(() => {
	// Drop the own property so `document.cookie` falls back to the browser's.
...
describe("purgeForeignClerkCookies", () => {
	it("expires each stale cookie host-only and on every candidate domain", async () => {
		...
		expect(writes).toEqual([
			...
			`__client_uat_lpBWIqOU=;path=/;domain=localhost;${EXPIRY}`,
			`__client_uat=;path=/;${EXPIRY}`,
			`__client_uat=;path=/;domain=localhost;${EXPIRY}`,
		])
