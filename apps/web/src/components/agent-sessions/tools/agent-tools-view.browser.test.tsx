...
describe("ToolDetailView", () => {
	it("names the tool and counts its calls and sessions", () => {
		...
		expect(screen.getAllByText(new RegExp(`${data.totals.sessions} sessions`)).length).toBeGreaterThan(0)
	})
...
describe("ToolErrorModal", () => {
	...
	it("lists the variants a group folded, and filters the samples by one", () => {
		...
		renderModal({
			match: (row) => row.message.includes("Expected array") && row.message.includes('\\"evidence\\"]'),
		})
	...
	it("narrows the samples to a session, and back out again", () => {
		...
		fireEvent.click(
			screen.getByRole("button", { name: `Show samples in ${sessionRowId(first.sessionId)}` }),
		)
	...
	it("links a session to its trace view without a window, and a sample to its span", () => {
		...
		expect(searches).toContainEqual({
			tool: "submit_candidate",
			view: "trace",
			span: built.occurrences[0]!.spanId,
		})
	})
