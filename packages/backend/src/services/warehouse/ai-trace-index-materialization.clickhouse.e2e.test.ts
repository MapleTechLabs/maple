...
describe.skipIf(!clickhouseE2eEnabled)("ai_trace_index materialization", () => {
	...
	it("materializes exactly the vendor-stamped spans, column by column", async () => {
		...
		assert.deepStrictEqual(rows, [
			...
			indexRow(TOOL_FAILURE_ORG_ID, STATUS_ONLY_FAILURE_SPAN, {
				ToolName: "submit_findings",
				IsToolCall: 1,
			}),
	...
	it("fingerprints a failure by its tool call result, else by its status message", async () => {
		...
		const fingerprint = (span: SeedSpan) =>
			rows.find((row) => row.SpanId === span.spanId)?.ErrorFingerprint
		const hashOf = async (row: {
		...
		assert.strictEqual(
			...
			await hashOf({
				failedToolCallResult: "",
				statusMessage: STATUS_ONLY_FAILURE_SPAN.statusMessage ?? "",
			}),
		)
