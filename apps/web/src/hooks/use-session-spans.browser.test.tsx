...
import { agentSpan, llmSpan, makeSpan } from "@maple/agent-sessions/testing"
import { useSessionSpans, type SessionSpansReads } from "./use-session-spans"

const mocks = {
	firstPage: {
		data: [] as readonly AiSessionSpan[],
		nextCursor: undefined as { timestamp: string; spanId: string } | undefined,
	},
}
...
describe("useSessionSpans", () => {
	...
	it("drains the agent's pages and then the app's on its own, after the first page's cursor", async () => {
		...
		fetchPage
			...
			.mockResolvedValueOnce({
				data: [agentSpan({ spanId: "agent-3", startMs: 120 * SECOND, durationMs: SECOND })],
	...
	it("drops the pages of a read the window moved on from, and a response landing late", async () => {
		...
		await act(async () => {
			resolveLate({
				data: [agentSpan({ spanId: "late", startMs: 0, durationMs: SECOND })],
				nextCursor: undefined,
			})
		})
