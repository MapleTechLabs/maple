	...
	content: ({ service_name, time_range }) =>
		Effect.succeed(
			`Triage an incident${service_name ? ` affecting service "${service_name}"` : ""}${time_range ? ` over the last ${time_range}` : ""}:\n\n` +
				...
				`5. Call \`diagnose_service\`${service_name ? ` with service="${service_name}"` : " for the most affected service"} for deep health analysis\n` +
				`6. Call \`service_map\`${service_name ? ` with service="${service_name}"` : ""} to understand the blast radius and upstream/downstream impact\n` +
